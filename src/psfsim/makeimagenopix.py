...
if __name__ == "__main__":
    ...
    for i in range(cat.nobjects):
        ...
        if not args.randomPos:
            ...
            if (
                imageCenter.x >= mybounds.getXMax()
                or imageCenter.y >= mybounds.getYMax()
                or imageCenter.x <= mybounds.getXMin()
                or imageCenter.y <= mybounds.getYMin()
            ):
                continue  # Skip item if out of bounds
            pos_SCA = galsim.PositionD(
                x=imageCenter.x - (mybounds.getXMax() / 2.0), y=imageCenter.y - (mybounds.getYMax() / 2.0)
            )
